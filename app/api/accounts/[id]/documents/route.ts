...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const bucket = (env as unknown as { FILES: R2Bucket }).FILES; await bucket.delete(row.r2_key); await db.batch([db.prepare("DELETE FROM document_chunks WHERE document_id = ? AND discovery_id = ?").bind(documentId, id), db.prepare("DELETE FROM documents WHERE id = ? AND discovery_id = ?").bind(documentId, id), db.prepare("DELETE FROM account_events WHERE source_type = 'document' AND source_id = ? AND discovery_id = ?").bind(documentId, id)]); await recomputeAccount(db, id); return Response.json({ ok: true });
}
