...
export async function GET(
  ...
  const payload = (await response.json()) as {
    guidedDiscoveries?: Array<Record<string, unknown>>;
  };
  const guidedDiscovery =
    payload.guidedDiscoveries?.find((item) => item.discoveryId === id) || null;
  return localizedJson(locale, { guidedDiscovery });
}
